import 'dart:ui';

const darkBlue = const Color(0xff1D5D9B);
const lightBlue = const Color(0xff75C2F6);
const yellow = const Color(0xffF4D160);
const lightShade = const Color(0xffFBEEAC);
const stroke = const Color(0xffD9D9D9);
const grey = const Color(0xff758694);
const pink = const Color(0xffE90074);
const shade = const Color(0xffD6BD98);
const white = const Color(0xffFFFFFF);
const red = const Color(0xffff3333);


//product and service container
const skyBlue = const Color(0xffCAEEFF);
const lightPink = const Color(0xffFFE1F7);
const lightGreen = const Color(0xff94FFD8);
const sandal = const Color(0xffFFEFD8);
const crimson = const Color(0xffCC4333);


//Package
const darkWhite = const Color(0xffEEEEEE);